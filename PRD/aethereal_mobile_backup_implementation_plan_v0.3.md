...
- Section 7: added `BACKUP_FAILED` outbound transitions
  (`-> PREFLIGHT_SCANNING`, `-> RECOVERY_REQUIRED`, `-> SOURCE_SAFE_TO_REMOVE`),
  closing a state-machine review finding where a failed job was an unrecoverable
  dead end. See the note in section 7.
- Section 14: documented intra-job duplicate-content handling
  (`LINK_NEW_OBJECT_SAME_JOB`) and `UNSUPPORTED` skip behavior, closing review
  finding R2-2 where two identical files in one job collided on the object path.
- Section 7: added `PREFLIGHT_BLOCKED` outbound transitions
  (`-> PREFLIGHT_SCANNING`, `-> IDLE`), closing a second state-machine dead end found
  while building the engine (a blocked preflight could not rescan or reset).
...
`BACKUP_FAILED` outbound transitions were added following a state-machine review
finding. Earlier drafts listed `BACKUP_FAILED` only as a transition target, never a
source, which made a failed job an unrecoverable dead end and contradicted WEB-004
(`Retry Failed Backup`, `Resume Interrupted Backup`). A failed job shall be able to:

- Retry through a fresh mandatory preflight (`-> PREFLIGHT_SCANNING`, per PRE-001).
- Enter reconciliation (`-> RECOVERY_REQUIRED`).
- Be abandoned cleanly so the source can be released (`-> SOURCE_SAFE_TO_REMOVE`).

`PREFLIGHT_BLOCKED` outbound transitions were added for the same reason: earlier drafts
listed it only as a target, leaving a blocked preflight unable to rescan or reset. A
blocked preflight shall be able to rescan (`-> PREFLIGHT_SCANNING`, e.g. after the user
frees destination space or swaps the SSD) or be abandoned (`-> IDLE`). PRE-008 forbids
*overriding* an insufficient-capacity block, not rescanning after the situation changes.
...
Intra-job duplicate content (review finding R2-2): when two or more source files in the
same job share a content identity, only the first `NEW` occurrence is planned as
`COPY_VERIFY_OBJECT_AND_LINK`. Subsequent occurrences are planned as
`LINK_NEW_OBJECT_SAME_JOB` and hardlink to the canonical object the first occurrence
produces, so identical content is never copied twice and the second instance cannot
collide on the content-addressed object path. Such duplicates are counted once toward
new-content bytes for capacity planning.
...
`UNSUPPORTED` objects (symlinks and special files, FILE-001) shall be skipped, not
copied, and shall not block the job.
